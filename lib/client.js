			...
			return {
				skills: {
					...
					async setInvocation(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("skills.setInvocation", {
							...input,
							cwd
						}));
					},
					...
					async importToProject(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("skills.importToProject", {
							...input,
							cwd
						}));
					},
					...
					async disableInProject(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("skills.disableInProject", {
							...input,
							cwd
						}));
					},
					...
					async enableInProject(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("skills.enableInProject", {
							...input,
							cwd
						}));
					},
				...
				mcp: {
					...
					async importToProject(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("mcp.importToProject", {
							...input,
							cwd
						}));
					},
				...
				quickMessages: {
					...
					async importToProject(input) {
						const cwd = deps.currentWorkspaceCwd();
						return unwrap(await rpc("quick.importToProject", {
							...input,
							cwd
						}));
					}
				...
				overrides: {
					...
					async get() {
						const result = await rpc("overrides.get", { cwd: deps.currentWorkspaceCwd() });
						if (!result.ok) throw new Error(result.errors.join("; "));
						return result.value;
					},
					/** 切换某个全局能力在本项目的禁用状态（写回后由面板重拉列表刷新标记）。 */
					async toggle(domain, key) {
						return unwrap(await rpc("overrides.toggle", {
							cwd: deps.currentWorkspaceCwd(),
							domain,
							key
						}));
...
.skp-tag-error{color:var(--skp-error);background:color-mix(in srgb,var(--skp-error) 8%,transparent);}
.skp-tag-warn{color:var(--skp-warn);background:var(--skp-warn-soft);}
...
.skp-detail-head{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:10px;}
...
.skp-detail-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:18px;}
.skp-form .skp-detail-actions{margin-top:6px;}
.skp-invoke-btns{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}
...
.skp-state-on{color:var(--skp-success);background:var(--skp-success-soft);border-color:transparent;}
.skp-state-on:hover:not(:disabled){background:color-mix(in srgb,var(--skp-success) 18%,transparent);}
.skp-state-off{color:var(--skp-text-3);background:var(--skp-fill);border-color:transparent;}
.skp-state-off:hover:not(:disabled){background:var(--skp-hover);}
.skp-state-partial{color:var(--skp-accent);background:var(--skp-accent-soft);border-color:transparent;}
.skp-state-partial:hover:not(:disabled){background:color-mix(in srgb,var(--skp-accent) 18%,transparent);}
...
.skp-dot-partial{background:var(--skp-accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--skp-accent) 18%,transparent);}
...
.skp-composer-skill-row{display:flex;align-items:center;border-radius:10px;}
.skp-composer-skill-row:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(20,28,40,.05));}
.skp-composer-skill{flex:1;min-width:0;display:flex;align-items:center;gap:8px;min-height:40px;padding:8px 6px 8px 10px;border:none;border-radius:10px;background:none;font:inherit;font-size:14px;line-height:22px;color:var(--dsw-alias-label-primary,#1b1f26);text-align:left;cursor:pointer;transition:background-color .12s ease;}
...
.skp-composer-skill-send{flex:none;display:grid;place-items:center;width:28px;height:28px;margin:0 4px 0 0;padding:0;border:none;border-radius:8px;background:transparent;color:var(--dsw-alias-label-secondary,#5b6472);cursor:pointer;opacity:0;pointer-events:none;transition:opacity .12s ease,background-color .12s ease;}
.skp-composer-skill-row:hover .skp-composer-skill-send,.skp-composer-skill-row:focus-within .skp-composer-skill-send{opacity:1;pointer-events:auto;}
.skp-composer-skill-send:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover,rgba(20,28,40,.05));}
.skp-composer-skill-send:focus-visible{outline:none;box-shadow:0 0 0 2px var(--dsw-alias-border-l3,rgba(20,28,40,.16));}
.skp-composer-skill-send svg{display:block;}
...
`;
		...
		*/
		...
		//#endregion
		//#region src/shared/skill-sources.ts
		/**
		...
		*/
		...
		function hasWorkspaceLabel(workspace) {
			return workspace !== void 0 && workspace !== "（无工作区）";
		}
		/** source 是否属于"项目系"（决定分区归属，composer 弹层的分组口径与此一致）。 */
		const isProjectSource = isProjectSkillSource;
		/** 「项目 / 全局」两区 Tab 行（三域视图头部共用，保证切换 UI 与文案完全一致）。 */
		function ZoneTabs({ value, onChange }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "skp-tabs",
				role: "tablist",
				children: ["project", "global"].map((z) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					role: "tab",
					"aria-selected": value === z,
					className: value === z ? "skp-tab skp-tab-active" : "skp-tab",
					onClick: () => onChange(z),
					children: z === "project" ? "项目" : "全局"
				}, z))
			});
		}
		/**
		...
		*/
		function useAsyncList(load, deps) {
		...
		//#endregion
		//#region src/client/mcp-common.ts
		...
		*/
		function aggregateMounts(statuses) {
			const byKey = {};
			...
			for (const status of statuses) for (const mount of status.servers) {
				...
				if (prev.state === "mounted") continue;
				if (rank(next.state) > rank(prev.state)) byKey[mount.key] = next;
			}
			return byKey;
		}
		//#endregion
		...
		*/
		function rowDotTitle(server, mount) {
			...
			if (mount === void 0) return "已启用 · 未挂载到当前会话";
			return mount.error !== void 0 ? `已启用 · ${MOUNT_LABEL[mount.state]}：${mount.error}` : `已启用 · ${MOUNT_LABEL[mount.state]}`;
		}
		/**
		...
		*/
		function McpView({ api, workspace }) {
			...
			const [zone, setZone] = (0, react.useState)("project");
			...
			const [confirmImport, setConfirmImport] = (0, react.useState)(void 0);
			/**
			...
			*/
			const { data, loading, error, reload } = useAsyncList(async () => {
				...
				return {
					...
					mounts: aggregateMounts(statuses)
				};
			}, [mcpApi, workspace]);
			...
			*/
			const onImport = async (server) => {
				...
				const result = await mcpApi.importToProject({
					key: server.key,
					overwrite: confirmImport === id
				});
				if (!result.ok) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ZoneTabs, {
								value: zone,
								onChange: (z) => {
									setZone(z);
									setSelected(void 0);
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								title: zone === "project" && noWorkspace ? "项目区需要可写的工作区才能添加" : void 0,
								onClick: () => {
									setEditServer(void 0);
									setSelected(void 0);
									setAddOpen(true);
								},
								children: ["+ 添加", zone === "project" ? "到项目" : "到全局"]
							})
						]
					}),
					...
					!loading && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("ul", {
							...
							children: [visible.map((server) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											...
											children: [zone === "project" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											...
											children: [
												...
												server.enabled && mount?.state === "failed" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: "skp-tag skp-tag-error",
													children: "挂载失败"
												}),
												server.enabled && mount?.state === "conflict" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: "skp-tag skp-tag-warn",
													children: "冲突"
												}),
								...
								className: "skp-empty",
								children: zone === "project" ? "本项目还没有 MCP 服务器。可到「全局」区导入，或点「+ 添加到项目」。" : "还没有全局 MCP 服务器。点「+ 添加到全局」添加。"
							})]
							...
							children: selectedServer ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(McpDetail, {
								...
								confirmingDelete: confirmDelete === rowId$2(selectedServer),
								confirmingImport: confirmImport === rowId$2(selectedServer),
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: server.key }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: isGlobal || imported ? "skp-tag skp-tag-global" : "skp-tag skp-tag-project",
							title: isGlobal ? server.shadowed ? "全局服务器（对所有项目生效）；当前被本项目内的同名条目遮蔽" : "全局服务器（对所有项目生效）" : isReference ? "本项目内对全局服务器的引用（内容跟随全局，启停只作用于本项目）" : imported ? "本项目内来自全局导入的副本（遮蔽全局同名条目，控制只作用于本项目）" : "本项目内的服务器（遮蔽全局同名条目）",
							children: isGlobal || imported ? "全局" : "项目"
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "skp-detail-actions",
						children: [
							!isGlobal && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: `skp-btn ${server.enabled ? "skp-state-on" : "skp-state-off"}`,
								title: isReference ? server.enabled ? "点击在本项目禁用该引用（本项目不再挂载，全局配置不变）" : "点击在本项目启用该引用（全局配置不变）" : server.enabled ? "点击禁用（保留在配置文件中，不挂载）" : "点击启用（写入配置文件并挂载）",
								onClick: onToggle,
								children: server.enabled ? "已启用" : "已禁用"
							}),
							isGlobal && !server.enabled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
							...
							isGlobal && hasWorkspace && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: confirmingImport ? "skp-btn skp-btn-danger" : "skp-btn",
								title: confirmingImport ? "项目内已有同名引用，再次点击将重新登记（保留原启停状态，全局配置不变）" : "在本项目登记对全局服务器的引用（内容跟随全局，可在本项目启停）",
								onClick: onImport,
								children: confirmingImport ? "确认重复导入同名引用？" : "导入到本项目"
							}),
							!isReference && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "skp-btn",
								onClick: onEdit,
								children: "编辑"
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								title: isGlobal ? "删除此全局服务器；各项目内的引用不受影响（变为悬空引用）" : isReference ? "移出本项目（移除对全局服务器的引用，全局原版仍在全局区）" : imported ? "移出本项目（移除全局导入的副本，全局原版仍在全局区）" : "删除本项目中的这个服务器",
								onClick: onDelete,
								children: confirmingDelete ? `确认${removeVerb}？` : removeVerb
							})
						]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: mount === void 0 ? "未挂载到当前会话" : MOUNT_LABEL[mount.state] }),
		...
		*/
		function McpForm({ mode, server, workspace, embedded, actionsClass = "skp-detail-actions", defaultScope, onCancel, onSave }) {
			...
			const [scope, setScope] = (0, react.useState)(server?.scope ?? defaultScope ?? (workspace !== void 0 ? "project" : "global"));
    ...
    "web": { "type": "http", "url": "http://localhost:3000/mcp" }
		...
		*/
		function McpJsonImport({ api, servers, workspace, defaultScope, onClose, onMutated }) {
			...
			const [scope, setScope] = (0, react.useState)(defaultScope ?? (noWorkspace ? "global" : "project"));
		...
		function QuickMessagesPanel({ api, workspace }) {
			...
			*/
			const onImport = async (message) => {
				...
				const result = await quickApi.importToProject({
					name: message.name,
					overwrite: confirmImport === id
				});
				if (!result.ok) {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								className: "skp-btn skp-btn-primary",
								disabled: zone === "project" && noWorkspace,
								title: zone === "project" && noWorkspace ? "项目区需要可写的工作区才能新增" : void 0,
								onClick: () => {
									setEditMessage(void 0);
									setSelected(void 0);
									setAddOpen(true);
								},
								children: ["+ 新增", zone === "project" ? "到项目" : "到全局"]
							})
						]
					}),
					listErrors.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					!loading && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("ul", {
							...
							children: [visible.map((message) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: "skp-row-meta",
											children: !message.enabled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: "skp-tag skp-tag-readonly",
												children: "已禁用"
											})
										}),
								...
								className: "skp-empty",
								children: zone === "project" ? "本项目还没有快捷消息。可到「全局」区导入，或点「+ 新增到项目」。" : "还没有全局快捷消息。点「+ 新增到全局」添加。"
							})]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "skp-detail",
							children: selectedMessage ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(QuickMessageDetail, {
								...
								hasWorkspace: !noWorkspace,
								confirmingDelete: confirmDelete === rowId$1(selectedMessage),
								confirmingImport: confirmImport === rowId$1(selectedMessage),
		...
		*  副本上发生。 */
		function QuickMessageDetail({ message, hasWorkspace, confirmingDelete, confirmingImport, onEdit, onToggle, onDelete, onImport }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "skp-detail-head",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: message.name }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: isGlobal || imported ? "skp-tag skp-tag-global" : "skp-tag skp-tag-project",
							title: isGlobal ? "全局快捷消息（对所有项目生效）" : isReference ? "本项目内对全局消息的引用（内容跟随全局，启停只作用于本项目）" : imported ? "本项目内来自全局导入的副本（控制只作用于本项目）" : "本项目内的快捷消息",
							children: isGlobal || imported ? "全局" : "项目"
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							!isGlobal && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								title: isReference ? message.enabled ? "点击在本项目禁用该引用（全局配置不变）" : "点击在本项目启用该引用（全局配置不变）" : message.enabled ? "点击禁用（保留在配置文件中，从快捷弹层隐藏）" : "点击启用（出现在输入框快捷弹层）",
								onClick: onToggle,
								children: message.enabled ? "已启用" : "已禁用"
							}),
							isGlobal && !message.enabled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "skp-btn skp-state-off",
							...
							isGlobal && hasWorkspace && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: confirmingImport ? "skp-btn skp-btn-danger" : "skp-btn",
								title: confirmingImport ? "项目内已有同名引用，再次点击将重新登记（保留原启停状态，全局配置不变）" : "在本项目登记对全局消息的引用（内容跟随全局，可在本项目启停）",
								onClick: onImport,
								children: confirmingImport ? "确认重复导入同名引用？" : "导入到本项目"
							}),
							!isReference && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: confirmingDelete ? "skp-btn skp-btn-danger" : "skp-btn skp-btn-danger-ghost",
								title: isGlobal ? "删除此全局消息；各项目内的引用不受影响（变为悬空引用）" : isReference ? "移出本项目（移除对全局消息的引用，全局原版仍在全局区）" : imported ? "移出本项目（移除全局导入的副本，全局原版仍在全局区）" : "删除本项目中的这条消息",
								onClick: onDelete,
								children: confirmingDelete ? `确认${removeVerb}？` : removeVerb
							})
						]
		...
		function QuickMessageForm({ mode, message, workspace, defaultScope, onCancel, onSave }) {
			const isNew = mode === "new";
			const noWorkspace = !hasWorkspaceLabel(workspace);
			const [scope, setScope] = (0, react.useState)(message?.scope ?? defaultScope ?? (noWorkspace ? "global" : "project"));
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							options: [{
								value: "project",
								label: "项目（.agents/quick-messages.json）",
								disabled: noWorkspace
							}, {
								value: "global",
								label: "全局（~/.agents/quick-messages.json）"
							}],
							onChange: (value) => setScope(value === "project" ? "project" : "global")
						})]
					}),
		...
		*/
		...
		const MAX_EXTRACT_BYTES = 67108864;
		...
		async function unzip(archive) {
			...
			for (let i = 0; i < entryCount; i += 1) {
				...
				if (localOffset + 30 > view.byteLength || view.getUint32(localOffset, true) !== LOCAL_SIGNATURE) throw new Error(`corrupt zip: bad local header for "${name}"`);
				const localNameLength = view.getUint16(localOffset + 26, true);
				const localExtraLength = view.getUint16(localOffset + 28, true);
				const dataStart = localOffset + 30 + localNameLength + localExtraLength;
				if (compressedSize > view.byteLength - dataStart) throw new Error(`corrupt zip: data overruns archive for "${name}"`);
				const compressed = archive.subarray(dataStart, dataStart + compressedSize);
				let data;
				if (method === 0) {
					if (totalBytes + compressedSize > MAX_EXTRACT_BYTES) throw new Error(`extracted data exceeds ${MAX_EXTRACT_BYTES} bytes (zip bomb?)`);
					data = compressed.slice();
				} else if (method === 8) data = await readWithLimit(new Blob([compressed]).stream().pipeThrough(new DecompressionStream("deflate-raw")), MAX_EXTRACT_BYTES - totalBytes, name);
		...
		function findEocd(view) {
			if (view.byteLength < 22) throw new Error("not a zip archive (too short)");
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								className: "skp-btn skp-btn-primary",
								disabled: zone === "project" && noWorkspace,
								title: zone === "project" && noWorkspace ? "项目区需要可写的工作区才能安装" : void 0,
								onClick: () => setInstallOpen(true),
								children: ["+ 安装", zone === "project" ? "到项目" : "到全局"]
							})
						]
					}),
					loading && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					!loading && !error && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("ul", {
							...
							children: [visible.map((item) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										...
										children: [zone === "project" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											title: state === "none" ? item.shadowStub === true ? "屏蔽占位：同名全局技能在本项目不可用（删除本条目即可恢复）" : "已禁用（用户与模型都不可调用）" : state === "partial" ? item.modelInvocable ? "仅用户不可调用（模型仍会触发）" : "仅模型不可调用（用户可 /name 调用）" : item.readOnly ? "只读条目" : "已启用（模型与用户均可调用）"
										}), item.name]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: "skp-row-desc",
								...
								className: "skp-empty",
								children: zone === "project" ? "本项目还没有技能。可到「全局」区导入，或点「+ 安装到项目」。" : "还没有全局技能。点「+ 安装到全局」添加。"
							})]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "skp-detail",
		...
		*/
		function SkillDetail({ summary, api, hasWorkspace, onChanged, onRemoved }) {
			...
			const [confirmImport, setConfirmImport] = (0, react.useState)(false);
			...
			*/
			const doSetInvocation = async (modelInvocable, userInvocable) => {
				...
				setOpError(void 0);
				try {
					const result = await api.setInvocation({
						...skillRef(summary),
						modelInvocable,
						userInvocable
					});
			...
			*/
			const doImport = async (overwrite) => {
				...
				setOpError(void 0);
				try {
					const result = await api.importToProject({
						name: summary.name,
						fromRoot: summary.root,
						overwrite
					});
					if (!result.ok) {
						if (result.existed === true) {
			...
			*/
			const doDisableInProject = async () => {
				...
				setOpError(void 0);
				try {
					const result = await api.disableInProject({
						name: summary.name,
						fromRoot: summary.root
					});
					if (!result.ok) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "skp-detail-head",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: summary.name }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: isGlobal ? "skp-tag skp-tag-global" : "skp-tag skp-tag-project",
								title: isGlobal ? "全局技能（对所有项目生效）" : "本项目内的技能",
								children: isGlobal ? "全局" : "项目"
							}),
							summary.readOnly && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "skp-badge",
								children: "只读"
							}),
							summary.shadowStub === true && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "skp-badge",
								title: "屏蔽占位：使同名全局技能在本项目不可用；删除本条目即可恢复",
								children: "屏蔽占位"
							}),
							isGlobal && summary.projectShadow === "stub" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "skp-badge",
								title: "本项目内已放置屏蔽占位，此技能在本项目不可用（不影响其它项目）",
								children: "本项目已禁用"
							}),
							isGlobal && summary.projectShadow === "skill" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "skp-badge",
								title: "项目区有同名技能，按宿主优先级在本项目生效的是项目副本",
								children: "项目副本生效中"
							})
						]
					}),
					!summary.readOnly && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "skp-detail-actions",
						children: [
							!isGlobal && summary.shadowStub !== true && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								...
								children: state === "full" ? "已启用" : state === "none" ? "已禁用" : "部分禁用"
							}),
							isGlobal && state !== "full" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "skp-btn skp-state-off",
								disabled: busy,
							...
							hasWorkspace && isGlobal && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: confirmImport ? "skp-btn skp-btn-danger" : "skp-btn",
								disabled: busy,
								title: confirmImport ? "项目内已有同名技能，再次点击将覆盖为全局副本" : "复制到本项目（.agents/skills），此后可在本项目内独立启停",
								onClick: () => void doImport(confirmImport),
								children: confirmImport ? "确认覆盖项目内的同名技能？" : "导入到本项目"
							}),
							hasWorkspace && isGlobal && summary.projectShadow === void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "skp-btn",
								disabled: busy,
							...
							hasWorkspace && isGlobal && summary.projectShadow === "stub" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "skp-btn",
								disabled: busy,
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								disabled: busy,
								title: summary.shadowStub === true ? "移除此屏蔽占位，恢复同名全局技能在本项目可用" : isGlobal ? "删除此全局技能（所有项目都不再可用）" : "删除本项目中的这份技能",
								onClick: doRemove,
								children: confirmRemove ? "确认删除？" : "删除"
							})
						]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
						...
						children: [
							...
							(summary.readOnly || !isGlobal) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: "调用方式" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: summary.readOnly || summary.shadowStub === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
								"模型：",
								summary.modelInvocable ? "✓" : "✗",
								" · 用户：",
								summary.userInvocable ? "✓" : "✗"
							] }) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "skp-invoke-btns",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									type: "button",
									className: `skp-btn ${summary.modelInvocable ? "skp-state-on" : "skp-state-off"}`,
									disabled: busy,
									title: summary.modelInvocable ? "点击关闭模型调用（agent 不再自动触发；用户仍可 /name 调用）" : "点击开启模型调用（agent 可自动触发）",
									onClick: () => void doSetInvocation(!summary.modelInvocable, summary.userInvocable),
									children: ["模型调用 ", summary.modelInvocable ? "✓" : "✗"]
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									type: "button",
									className: `skp-btn ${summary.userInvocable ? "skp-state-on" : "skp-state-off"}`,
									disabled: busy,
									title: summary.userInvocable ? "点击关闭用户调用（输入框 /name 不再注入此技能；模型仍可自动触发）" : "点击开启用户调用（输入框 /name 可注入此技能）",
									onClick: () => void doSetInvocation(summary.modelInvocable, !summary.userInvocable),
									children: ["用户调用 ", summary.userInvocable ? "✓" : "✗"]
								})]
							}) })] }),
		...
		*/
		function InstallDialog({ api, zone, hasWorkspace, onClose, onInstalled }) {
			...
			const [scopeChoice, setScopeChoice] = (0, react.useState)(zone === "global" ? "global" : "project-agents");
			const [overwrite, setOverwrite] = (0, react.useState)(false);
			const [busy, setBusy] = (0, react.useState)(false);
			const [errors, setErrors] = (0, react.useState)([]);
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								options: zone === "global" ? [{
									value: "global",
									label: "全局 — ~/.agents/skills"
								}] : [{
									value: "project-agents",
									label: "项目 — .agents/skills（默认）",
									disabled: !hasWorkspace
								}, {
									value: "project-dsh",
									label: "项目 — .dsh/skills（旧）",
									disabled: !hasWorkspace
								}],
		...
		//#endregion
		//#region src/client/composer-common.ts
		/**
		...
		*/
		...
		function createComposerStore() {
			...
			return {
				...
				isFresh(workspace) {
					return data !== void 0 && dataToken === token && dataWorkspace === workspace;
				},
				subscribe(listener) {
		...
		function useComposerStore(store) {
			const [, force] = (0, react.useState)(0);
			(0, react.useEffect)(() => store.subscribe(() => force((n) => n + 1)), [store]);
		...
		*/
		function useWorkspaceLabel(api) {
			const [workspace, setWorkspace] = (0, react.useState)(() => api.workspaceLabel());
		...
		*/
		function useComposerDismiss(store, open, popRef, ownButtonClass) {
			(0, react.useEffect)(() => {
				if (!open) return;
				const onKeyDown = (event) => {
				...
				const onPointerDown = (event) => {
					...
					if (target instanceof Element && target.closest(ownButtonClass) !== null) return;
					store.setOpen(false);
				};
				document.addEventListener("pointerdown", onPointerDown, true);
				return () => {
		...
		*/
		function composerPopStyle(anchor) {
		...
		*/
		function useComposerData(state, workspace) {
			return state.dataToken === state.token && state.dataWorkspace === workspace ? state.data : void 0;
		}
		//#endregion
		...
		*/
		function ComposerMcpButton({ api }) {
			const state = useComposerStore(store$2);
			const workspace = useWorkspaceLabel(api);
			const list = useComposerData(state, workspace);
			(0, react.useEffect)(() => {
				if (state.open) return;
				if (store$2.isFresh(workspace)) return;
				let cancelled = false;
				api.mcp.list().then((fresh) => {
					if (!cancelled) store$2.setData(workspace, fresh);
				}).catch(() => void 0);
				return () => {
					cancelled = true;
				};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HammerIcon, { filled: active })
			});
		}
		/** 复合行 id：同一个 key 可能同时存在于 project 与 global 两个作用域。 */
		...
		*/
		async function toggleServer(api, server) {
			if (server.scope === "project") return api.mcp.setEnabled({
				scope: "project",
				key: server.key,
				enabled: !server.enabled
			});
		...
		function switchTitle(server) {
			if (server.shadowed) return "被项目级同名条目遮蔽（切换只影响配置文件，本项目内不生效）";
			if (server.scope === "project") return server.enabled ? "点击禁用" : "点击启用";
			return server.disabledInProject === true ? "点击在本项目启用（移除项目级禁用声明，全局配置不变）" : "点击在本项目禁用（全局配置不变，其他项目不受影响）";
		}
		/**
		...
		*/
		function ComposerMcpOverlay({ api }) {
			...
			useComposerDismiss(store$2, state.open, popRef, ".skp-composer-btn-mcp");
			(0, react.useEffect)(() => {
				if (!state.open) return;
				let cancelled = false;
				setLoading(true);
				...
				Promise.all([api.mcp.list(), api.mcp.status().catch(() => [])]).then(([list, statuses]) => {
					if (cancelled) return;
					store$2.setData(workspace, list);
			...
			*  落盘后宿主自动重挂受影响 session，再 bump 修订号让弹层与按钮一起重拉。 */
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							title: server.disabledInProject === true ? "本项目禁用（本项目的 session 不挂载）" : mount === void 0 ? "未挂载到当前会话" : mount.error ?? MOUNT_LABEL[mount.state]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				role: "dialog",
				"aria-label": "MCP 服务器",
				style: composerPopStyle(state.anchor),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					loading && servers.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: [
							...
							" 或全局 ",
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: "~/.agents/mcp.json" }),
		...
		*/
		const store$1 = createComposerStore();
		/** 切换（或显式设置）弹层开合；打开时 bump token 触发弹层重拉。 */
		function setComposerSkillsOpen(open) {
			store$1.setOpen(open);
		}
		/** 记录能力工具组的视口位置（在打开弹层前调用）。 */
		function setComposerSkillsAnchor(rect) {
			store$1.setAnchor(rect);
		}
		...
		/**
		* 合并去重后的 user-invocable skill 列表（项目 + 全局）。
		*
		* 可见性规则与宿主 registry 的解析口径逐字一致——**同名以第一个出现的
		* 条目为准，无论它是否可用**（列表顺序本身就是项目根在前、全局根在后，
		* 与 registry 的 rank 解析同序，见 skills/manager.ts 的 list 排序）：
		*
		*   - 第一个可用（userInvocable）→ 收录；
		*   - 第一个被禁用（项目副本 / shadow stub / 项目原生条目）→ 整个名字排除。
		*     此时宿主解析 `/name` 命中的也是这个被禁用的条目（rank 优先），不注入；
		*     若放行全局同名条目进弹层，用户点到的就是死口令。
		*
		* `.claude` 兼容根（面板只读展示）不参与宿主 registry（rank 表里没有它），
		* 其条目一律排除——列出来同样是死口令。
		*/
		function invocableSkills(list) {
			...
			for (const item of list) {
				if (item.source === "project-claude" || item.source === "user-claude") continue;
		...
		*/
		...
		const SEND_PATH$1 = "M8.3125 0.980183C8.66767 1.0531 8.97902 1.20418 9.2627 1.43233C9.48724 1.61297 9.73029 1.85793 9.97949 2.10714L14.707 6.83468L13.293 8.24874L9 3.95577V15.0417H7V3.95577L2.70703 8.24874L1.29297 6.83468L6.02051 2.10714C6.26971 1.85793 6.51277 1.61297 6.7373 1.43233C6.97662 1.23986 7.28445 1.04402 7.6875 0.980183C7.8973 0.947006 8.1031 0.95516 8.3125 0.980183Z";
		function SendIcon$1() {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
				width: "16",
				height: "16",
		...
		*/
		function ComposerSkillsButton({ api, draft }) {
			const state = useComposerStore(store$1);
			const workspace = useWorkspaceLabel(api);
			const list = useComposerData(state, workspace);
			const names = (0, react.useMemo)(() => list === void 0 ? [] : invocableSkills(list).map((s) => s.name), [list]);
			(0, react.useEffect)(() => {
				if (state.open) return;
				if (store$1.isFresh(workspace)) return;
				let cancelled = false;
				api.skills.list().then((fresh) => {
					if (!cancelled) store$1.setData(workspace, fresh);
				}).catch(() => void 0);
				return () => {
					cancelled = true;
				};
		...
		*/
		function ComposerSkillsOverlay({ api, useInput, inputActions }) {
			...
			const [query, setQuery] = (0, react.useState)("");
			const popRef = (0, react.useRef)(null);
			useComposerDismiss(store$1, state.open, popRef, ".skp-composer-btn-skills");
			(0, react.useEffect)(() => {
				if (!state.open) {
					setQuery("");
					return;
				...
				api.skills.list().then((list) => {
					if (!cancelled) {
						store$1.setData(workspace, list);
		...
		function SkillsPop({ anchor, popRef, skills, loading, error, query, setQuery, useInput, inputActions }) {
			...
			*/
			...
			const renderRow = (skill) => {
				...
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "skp-composer-skill-row",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SendIcon$1, {})
					})]
				}, skill.name);
			};
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					loading && skills.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: skills.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("br", {}),
							"在项目 ",
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: ".agents/skills" }),
							" 或全局 ",
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: "~/.agents/skills" }),
							" 中添加，或在能力面板中安装。"
						] }) : "没有匹配项。"
