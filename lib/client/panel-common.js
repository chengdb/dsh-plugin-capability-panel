...
export function isProjectSource(source) {
...
export function ScopeTabs({ value, onChange }) {
    return (_jsx("div", { className: "skp-tabs", role: "tablist", children: ["all", "project", "global"].map((t) => (_jsx("button", { role: "tab", "aria-selected": value === t, className: value === t ? "skp-tab skp-tab-active" : "skp-tab", onClick: () => onChange(t), children: SCOPE_LABEL[t] }, t))) }));
}
/**
 ...
 * （本项目状态没有意义，先启用全局配置再说）。
 */
export function ProjectOverrideButton({ globalEnabled, projectDisabled, disabled, actionWord = "生效", onToggle, }) {
    return (_jsx("button", { type: "button", className: `skp-btn ${!globalEnabled ? "skp-state-off" : projectDisabled ? "skp-state-proj" : "skp-state-on"}`, disabled: disabled === true || !globalEnabled, title: !globalEnabled
            ? "全局已禁用：先启用全局配置，本项目状态才有意义"
            : projectDisabled
                ? `点击恢复在本项目${actionWord}（全局配置不变）`
                : "点击在本项目禁用（仅本项目生效，全局配置不变）", onClick: onToggle, children: projectDisabled ? "项目已禁用" : "项目已启用" }));
}
/**
 * 三域视图共用的"拉取 + 重拉"状态机。
 *
