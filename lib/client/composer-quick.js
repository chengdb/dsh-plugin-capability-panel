...
export function ComposerQuickButton() {
    ...
    const className = ["skp-composer-btn", "skp-composer-btn-quick", open ? "skp-composer-btn-open" : ""]
        .filter(Boolean)
        .join(" ");
    return (_jsx("button", { type: "button", className: className, title: "\u5FEB\u6377\u6D88\u606F", "aria-label": "\u5FEB\u6377\u6D88\u606F", "aria-expanded": open, onClick: (event) => {
...
export function ComposerQuickOverlay({ api, useInput, inputActions, }) {
    ...
    useEffect(() => {
        ...
        api.quickMessages
            ...
            .then((list) => {
            if (!cancelled)
                setMessages(list.messages.filter((m) => m.enabled));
        })
            .catch((err) => {
            if (!cancelled)
                setError(String(err));
    ...
    useEffect(() => {
        ...
        const onPointerDown = (event) => {
            ...
            if (popRef.current?.contains(target) === true)
                return;
            if (target instanceof Element && target.closest(".skp-composer-btn-quick") !== null)
                return;
            setComposerQuickOpen(false);
        };
    ...
    if (!open)
        return null;
    return (_jsx(QuickPop, { anchor: anchor, popRef: popRef, messages: messages, loading: loading, error: error, query: query, setQuery: setQuery, useInput: useInput, inputActions: inputActions }));
}
/**
...
function QuickPop({ anchor, popRef, messages, loading, error, query, setQuery, useInput, inputActions, }) {
    ...
    const renderRow = (message) => (_jsxs("button", { type: "button", className: "skp-composer-skill", disabled: !canInsert, title: canInsert ? `输入「${message.name}」` : "当前会话不支持快速输入", onClick: () => onPick(message), children: [_jsx("span", { className: "skp-composer-skill-name", children: message.name }), _jsx("span", { className: "skp-composer-skill-desc", children: message.text })] }, `${message.scope}:${message.name}`));
    return (_jsxs("div", { ref: popRef, className: "skp-composer-pop skp-composer-pop-quick", role: "dialog", "aria-label": "\u5FEB\u6377\u6D88\u606F", style: anchor === undefined
            ? undefined
            : {
                position: "fixed",
                left: anchor.left,
