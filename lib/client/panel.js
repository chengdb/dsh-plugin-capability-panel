...
function SkillsView({ api, workspace }) {
    ...
    return (_jsxs("div", { className: "skp-domain", children: [_jsxs("div", { className: "skp-subheader", children: [_jsx(ScopeTabs, { value: tab, onChange: setTab }), _jsx("input", { className: "skp-search", type: "search", placeholder: "\u641C\u7D22\u6280\u80FD\u2026", value: query, onChange: (e) => setQuery(e.target.value) }), _jsx("button", { type: "button", className: "skp-btn skp-btn-primary", onClick: () => setInstallOpen(true), children: "+ \u5B89\u88C5" })] }), loading && _jsx("div", { className: "skp-status", children: "\u52A0\u8F7D\u4E2D\u2026" }), error && _jsx("div", { className: "skp-error", children: error }), !loading && !error && (_jsxs("div", { className: "skp-body", children: [_jsxs("ul", { className: "skp-list", children: [visible.map((item) => (_jsx("li", { children: _jsxs("button", { className: selected === skillRowKey(item) ? "skp-row skp-row-active" : "skp-row", onClick: () => setSelected(skillRowKey(item)), children: [_jsxs("span", { className: "skp-row-name", children: [_jsx("span", { className: `skp-dot ${!isSkillEnabled(item) ? "" : item.disabledInProject === true ? "skp-dot-project-disabled" : "skp-dot-enabled"}`, title: !isSkillEnabled(item)
                                                        ? "已禁用（用户与模型都不可调用）"
                                                        : item.disabledInProject === true
                                                            ? "本项目禁用（全局仍启用）"
                                                            : item.readOnly
                                                                ? "只读条目"
                                                                : "已启用（模型与用户均可调用）" }), item.name] }), _jsxs("span", { className: "skp-row-meta", children: [_jsx("span", { className: isProjectSource(item.source) ? "skp-tag skp-tag-project" : "skp-tag skp-tag-global", children: isProjectSource(item.source) ? "项目" : "全局" }), item.readOnly ? (_jsx("span", { className: "skp-tag skp-tag-readonly", children: "\u53EA\u8BFB" })) : (_jsx("span", { className: item.format === "directory" ? "skp-tag skp-tag-directory" : "skp-tag skp-tag-flat", children: item.format === "directory" ? "目录" : "单文件" })), !item.readOnly && !isSkillEnabled(item) && _jsx("span", { className: "skp-tag skp-tag-readonly", children: "\u5DF2\u7981\u7528" }), item.disabledInProject === true && _jsx("span", { className: "skp-tag skp-tag-project-disabled", children: "\u672C\u9879\u76EE\u7981\u7528" })] }), _jsx("span", { className: "skp-row-desc", children: item.description })] }) }, skillRowKey(item)))), visible.length === 0 && _jsx("li", { className: "skp-empty", children: "\u6CA1\u6709\u5339\u914D\u7684\u6280\u80FD\u3002" })] }), _jsx("div", { className: "skp-detail", children: selectedItem ? (_jsx(SkillDetail, { summary: selectedItem, api: skillsApi, overrides: api.overrides, hasWorkspace: hasWorkspaceLabel(workspace), 
                            ...
                            onRemoved: () => {
                                ...
                                reload();
                            } }, skillRowKey(selectedItem))) : (_jsx("div", { className: "skp-detail-empty", children: "\u9009\u62E9\u4E00\u9879\u6280\u80FD\u67E5\u770B\u8BE6\u60C5\u3002" })) })] })), installOpen && (_jsx(InstallDialog, { api: skillsApi, hasWorkspace: hasWorkspaceLabel(workspace), onClose: () => setInstallOpen(false), onInstalled: () => {
...
function SkillDetail({ summary, api, overrides, hasWorkspace, onChanged, onRemoved, }) {
    ...
    return (_jsxs("div", { className: "skp-detail-card", children: [_jsxs("div", { className: "skp-detail-head", children: [_jsx("h3", { children: summary.name }), summary.readOnly && _jsx("span", { className: "skp-badge", children: "\u53EA\u8BFB" })] }), !summary.readOnly && (_jsxs("div", { className: "skp-detail-actions", children: [_jsx("button", { type: "button", className: `skp-btn ${enabled ? "skp-state-on" : "skp-state-off"}`, disabled: busy, title: isGlobal
                            ? enabled
                                ? "点击全局禁用（所有项目都不可调用）"
                                : "点击全局启用（恢复所有项目可用）"
                            : enabled
                                ? "点击禁用（用户与模型都不可调用）"
                                : "点击启用（恢复用户与模型调用）", onClick: () => void doSetEnabled(!enabled), children: isGlobal ? (enabled ? "全局已启用" : "全局已禁用") : enabled ? "已启用" : "已禁用" }), hasWorkspace && isGlobal && (_jsx(ProjectOverrideButton, { globalEnabled: enabled, projectDisabled: projectDisabled, disabled: busy, onToggle: () => void doToggleProjectDisabled() })), _jsx("button", { type: "button", className: "skp-btn", disabled: busy, onClick: doExportDownload, children: "\u5BFC\u51FA" }), _jsx("button", { type: "button", className: "skp-btn", disabled: busy, onClick: () => setExportPathOpen(true), children: "\u5BFC\u51FA\u5230\u8DEF\u5F84\u2026" }), _jsx("button", { type: "button", className: confirmRemove ? "skp-btn skp-btn-danger" : "skp-btn skp-btn-danger-ghost", disabled: busy, onClick: doRemove, children: confirmRemove ? "确认移除？" : "移除" })] })), _jsxs("dl", { className: "skp-detail-fields", children: [_jsx("dt", { children: "\u63CF\u8FF0" }), _jsx("dd", { children: summary.description }), summary.whenToUse !== undefined && (_jsxs(_Fragment, { children: [_jsx("dt", { children: "\u4F7F\u7528\u65F6\u673A" }), _jsx("dd", { children: summary.whenToUse })] })), _jsx("dt", { children: "\u8C03\u7528\u65B9\u5F0F" }), _jsxs("dd", { children: ["\u6A21\u578B\uFF1A", summary.modelInvocable ? "✓" : "✗", " \u00B7 \u7528\u6237\uFF1A", summary.userInvocable ? "✓" : "✗"] }), _jsx("dt", { children: "\u6765\u6E90" }), _jsx("dd", { children: summary.source }), summary.path !== undefined && (_jsxs(_Fragment, { children: [_jsx("dt", { children: "\u8DEF\u5F84" }), _jsx("dd", { className: "skp-path", children: summary.path })] }))] }), opError !== undefined && _jsx("div", { className: "skp-error", children: opError }), exportPathOpen && (_jsx(ExportToPathDialog, { summary: summary, api: api, onClose: () => setExportPathOpen(false) }))] }));
}
/**
...
function InstallDialog({ api, hasWorkspace, onClose, onInstalled }) {
    ...
    const [scopeChoice, setScopeChoice] = useState(hasWorkspace ? "project-agents" : "global");
