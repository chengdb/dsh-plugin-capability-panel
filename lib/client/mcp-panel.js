...
export function McpView({ api, workspace }) {
        ...
        Promise.all([api.list(), api.status().catch(() => [])])
            ...
            for (const status of statuses) {
                for (const mount of status.servers) {
                    ...
                    if (prev === undefined || rank(next.state) > rank(prev.state))
    ...
    return (_jsxs("div", { className: "skp-domain", children: [_jsxs("div", { className: "skp-subheader", children: [_jsx("div", { className: "skp-tabs", role: "tablist", children: ["all", "project", "global"].map((t) => (_jsx("button", { role: "tab", "aria-selected": tab === t, className: tab === t ? "skp-tab skp-tab-active" : "skp-tab", onClick: () => setTab(t), children: SCOPE_LABEL[t] }, t))) }), _jsx("input", { className: "skp-search", type: "search", placeholder: "\u641C\u7D22 MCP \u670D\u52A1\u5668\u2026", value: query, onChange: (e) => setQuery(e.target.value) }), _jsx("button", { type: "button", className: "skp-btn skp-btn-primary", onClick: () => {
                            setEditServer(undefined);
                            setSelected(undefined);
                            setAddOpen(true);
                        }, children: "+ \u6DFB\u52A0\u670D\u52A1\u5668" })] }), listErrors.length > 0 && _jsx("div", { className: "skp-error", children: listErrors.join("\n") }), opError && _jsx("div", { className: "skp-error", children: opError }), loading && _jsx("div", { className: "skp-status", children: "\u52A0\u8F7D\u4E2D\u2026" }), !loading && (_jsxs("div", { className: "skp-body", children: [_jsxs("ul", { className: "skp-list", children: [visible.map((server) => {
                                const id = rowId(server);
                                ...
                                return (_jsx("li", { children: _jsxs("button", { className: selected === id ? "skp-row skp-row-active" : "skp-row", onClick: () => {
                                            ...
                                            setEditServer(undefined);
                                            setConfirmDelete(undefined);
                                        }, children: [_jsxs("span", { className: "skp-row-name", children: [_jsx("span", { className: `skp-dot skp-dot-${mount?.state ?? "none"}`, title: mount === undefined ? "未挂载到当前会话" : MOUNT_LABEL[mount.state] }), server.key] }), _jsxs("span", { className: "skp-row-meta", children: [_jsx("span", { className: server.scope === "project" ? "skp-tag skp-tag-project" : "skp-tag skp-tag-global", children: server.scope === "project" ? "项目" : "全局" }), _jsx("span", { className: "skp-tag skp-tag-flat", children: server.transport }), !server.enabled && _jsx("span", { className: "skp-tag skp-tag-readonly", children: "\u5DF2\u7981\u7528" }), server.shadowed && _jsx("span", { className: "skp-tag skp-tag-directory", children: "\u88AB\u906E\u853D" })] }), _jsx("span", { className: "skp-row-desc", children: server.summary })] }) }, id));
                            }), visible.length === 0 && _jsx("li", { className: "skp-empty", children: "\u6CA1\u6709\u5339\u914D\u7684 MCP \u670D\u52A1\u5668\u3002" })] }), _jsx("div", { className: "skp-detail", children: selectedServer ? (_jsx(McpDetail, { server: selectedServer, mount: mounts[selectedServer.key], confirming: confirmDelete === rowId(selectedServer), onEdit: () => setEditServer(selectedServer), onToggle: () => onToggle(selectedServer), onDelete: () => onDelete(selectedServer) })) : (_jsx("div", { className: "skp-detail-empty", children: "\u9009\u62E9\u4E00\u4E2A\u670D\u52A1\u5668\u67E5\u770B\u8BE6\u60C5\uFF0C\u6216\u65B0\u589E\u4E00\u4E2A\u3002" })) })] })), addOpen && (_jsx(McpAddDialog, { api: api, servers: servers, workspace: workspace, onClose: () => setAddOpen(false), onMutated: () => reload() })), editServer !== undefined && (_jsx(McpEditDialog, { api: api, server: editServer, workspace: workspace, onClose: () => setEditServer(undefined), onSaved: () => reload() }))] }));
}
// ---------------------------------------------------------------------------
// 详情卡片
// ---------------------------------------------------------------------------
