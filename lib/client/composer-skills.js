...
function invocableSkills(list) {
    ...
    for (const item of list) {
        if (!item.userInvocable || seen.has(item.name))
            continue;
        seen.add(item.name);
        result.push(item);
    }
...
export function ComposerSkillsButton({ api, draft }) {
    ...
    useEffect(() => {
        ...
        api.skills
            ...
            .then((list) => {
            if (!cancelled)
                setNames(invocableSkills(list).map((s) => s.name));
        })
            .catch(() => undefined);
        return () => {
            cancelled = true;
    ...
    const className = [
        ...
        .filter(Boolean)
        .join(" ");
    return (_jsx("button", { type: "button", className: className, title: "Skills", "aria-label": "Skills", "aria-expanded": open, onClick: (event) => {
...
export function ComposerSkillsOverlay({ api, useInput, inputActions, }) {
    ...
    useEffect(() => {
        ...
        const onPointerDown = (event) => {
            ...
            if (popRef.current?.contains(target) === true)
                return;
            if (target instanceof Element && target.closest(".skp-composer-btn-skills") !== null)
                return;
            setComposerSkillsOpen(false);
        };
    ...
    if (!open)
        return null;
    return (_jsx(SkillsPop, { anchor: anchor, popRef: popRef, skills: skills, loading: loading, error: error, query: query, setQuery: setQuery, useInput: useInput, inputActions: inputActions }));
}
/**
...
function SkillsPop({ anchor, popRef, skills, loading, error, query, setQuery, useInput, inputActions, }) {
    ...
    const renderRow = (skill) => (_jsxs("button", { type: "button", className: "skp-composer-skill", disabled: !canInsert, title: canInsert ? `输入 /${skill.name}` : "当前会话不支持快速输入", onClick: () => onPick(skill), children: [_jsx("span", { className: "skp-composer-skill-name", children: skill.name }), _jsx("span", { className: "skp-composer-skill-desc", children: skill.description })] }, skill.name));
    return (
    // 弹层左下角贴能力工具组左上角（上方间隔 4px）；无锚点时退化为锚点左上方位（CSS 类默认值）。
    _jsxs("div", { ref: popRef, className: "skp-composer-pop skp-composer-pop-skills", role: "dialog", "aria-label": "Skills", style: anchor === undefined
            ? undefined
            : {
                position: "fixed",
                left: anchor.left,
