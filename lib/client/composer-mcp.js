...
export function ComposerMcpButton({ api }) {
    ...
    useEffect(() => {
        ...
        api.mcp
            ...
            .then((list) => {
            if (!cancelled)
                setEnabledCount(list.servers.filter((s) => s.enabled && !s.shadowed).length);
        })
            .catch(() => undefined);
        return () => {
            cancelled = true;
    ...
    const className = [
        ...
        .filter(Boolean)
        .join(" ");
    return (_jsx("button", { type: "button", className: className, title: "MCP \u670D\u52A1\u5668", "aria-label": "MCP \u670D\u52A1\u5668", "aria-expanded": open, onClick: (event) => {
            ...
            setComposerMcpOpen();
        }, children: _jsx(HammerIcon, { filled: enabledCount > 0 }) }));
}
/** 挂载状态的圆点 tooltip 文案。 */
const MOUNT_LABEL = {
    mounted: "已挂载",
...
export function ComposerMcpOverlay({ api }) {
    ...
    useEffect(() => {
        ...
        const onPointerDown = (event) => {
            ...
            if (popRef.current?.contains(target) === true)
                return;
            if (target instanceof Element && target.closest(".skp-composer-btn-mcp") !== null)
                return;
            setComposerMcpOpen(false);
        };
    ...
    const renderRow = (server) => {
        ...
        return (_jsxs("div", { className: "skp-composer-row", children: [_jsx("span", { className: `skp-composer-dot skp-composer-dot-${mount?.state ?? "none"}`, title: mount === undefined ? "未挂载到当前会话" : mount.error ?? MOUNT_LABEL[mount.state] }), _jsx("span", { className: "skp-composer-name", children: server.key }), _jsxs("label", { className: "skp-switch", title: server.shadowed ? "被项目级同名条目遮蔽（切换只影响配置文件，本项目内不生效）" : server.enabled ? "点击禁用" : "点击启用", children: [_jsx("input", { type: "checkbox", checked: server.enabled, disabled: busy, onChange: () => void onToggle(server) }), _jsx("span", { className: "skp-switch-track" })] })] }, id));
    };
    return (
    // 弹层左下角贴能力工具组左上角（上方间隔 4px）；无锚点时退化为锚点左上方位（CSS 类默认值）。
    _jsxs("div", { ref: popRef, className: "skp-composer-pop", role: "dialog", "aria-label": "MCP \u670D\u52A1\u5668", style: anchor === undefined
            ? undefined
            : {
                position: "fixed",
                left: anchor.left,
