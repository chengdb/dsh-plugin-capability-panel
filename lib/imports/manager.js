...
import { IMPORT_DOMAINS } from "./types.js";
/** 创建管理服务。 */
export function createImportsManager() {
     ...
     */
    async function entries(cwd, domain) {
    ...
    function guard(input) {
        if (!IMPORT_DOMAINS.includes(input.domain))
            return { ok: false, errors: [`unknown domain "${String(input.domain)}"`] };
        if (input.name.trim().length === 0)
            return { ok: false, errors: ["name must not be empty"] };
        if (fileFor(input.cwd) === undefined)
     ...
     * 没有"内容"可覆盖，保留原启停状态）。
     */
    async function importRef(input) {
        ...
        return withLockedFile(input, (imports) => {
            ...
            if (!Object.hasOwn(table, input.name))
                table[input.name] = {};
            return { ok: true };
        });
    }
