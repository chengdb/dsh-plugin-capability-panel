 ...
 */
export async function readImports(filePath) {
    ...
    for (const domain of IMPORT_DOMAINS) {
        ...
        for (const [name, value] of Object.entries(table)) {
            ...
            out[domain][name] = normalizeRef(value);
        }
    }
    return out;
}
 ...
 */
export async function writeImports(filePath, imports) {
    const body = {};
    for (const domain of IMPORT_DOMAINS) {
        ...
        for (const name of names) {
            table[name] = normalizeRef(imports[domain][name]);
        }
        body[domain] = table;
    }
