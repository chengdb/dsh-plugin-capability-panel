 ...
 */
export declare function locateSkillRoot(paths: string[]): LocateResult;
/**
 * 若所有路径共享同一个首段（zipball 典型的 `<owner>-<repo>-<sha>/` 顶层
 * 文件夹），返回剥掉该首段后的路径列表；否则原样返回。
 */
export declare function stripCommonTopFolder(paths: string[]): string[];
/**
 * 按定位结果把一组文件裁剪并重定根：只保留 skill 根下的文件，路径改写为
 * 相对 skill 根。flat 布局只保留那个 .md 文件本身。
 */
export declare function rerootEntries<T extends {
    path: string;
}>(entries: T[], root: LocatedSkillRoot): T[];
