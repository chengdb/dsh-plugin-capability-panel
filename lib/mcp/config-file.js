...
import { transportOf } from "./entry-util.js";
 ...
 */
export async function readMcpFile(filePath) {
    let text;
    try {
 ...
 */
export async function writeMcpFile(filePath, servers) {
 ...
 * 回退到 `"server"`。
 */
export function sanitizeServerName(key) {
...
export function interpolateEnv(value, env = process.env) {
    return value.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)\}/g, (_match, name) => env[name] ?? "");
}
/**
 * 把文件条目映射成桥接层配置。
 *
 ...
 */
export function toClientConfig(key, entry, fallbackCwd) {
