...
export function mountRpcChannel(ctx, service) {
    const handler = (endpoint, payload, signal) => handleEndpoint(service, endpoint, payload, signal);
    ...
    let disposeHandle;
    const stopInject = ctx.inject(["connection"], (connectionCtx) => {
    ...
    return () => {
        if (typeof disposeHandle === "function")
            disposeHandle();
