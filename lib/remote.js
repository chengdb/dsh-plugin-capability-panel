 ...
 */
export async function handleEndpoint(service, endpoint, payload, _signal) {
    const p = (payload ?? {});
 ...
 * {@link RpcResult}。
 */
export function mountRpcChannel(ctx, service) {
    const handler = (endpoint, payload) => handleEndpoint(service, endpoint, payload);
    if (typeof ctx.inject !== "function") {
        ctx.logger?.warn?.("capability-panel: `ctx.inject` unavailable; host RPC channel not mounted");
        return () => undefined;
    }
    let disposeRoute;
    const stopInject = ctx.inject(["connection", "webServer"], (hostCtx) => {
        ...
        disposeRoute = hostCtx.effect(() => webServer.register({ kind: "prefix", path: CHANNEL, handler: createChannelRoute(connection, handler) }), `capability-panel: ${CHANNEL} rpc channel`);
    });
    return () => {
        if (typeof disposeRoute === "function")
            disposeRoute();
        // cordis 4 的 `ctx.inject` 返回 Fiber（对象，带 dispose()），旧版返回函数。
        if (typeof stopInject === "function")
            stopInject();
        else
            void stopInject?.dispose?.();
    };
}
 ...
 */
function createChannelRoute(connection, handler) {
    /** 写一个 JSON 响应。 */
    ...
    const badRequest = (res, rpcId, message) => respond(res, {
