 ...
 */
export function classifySkillUrl(input) {
    let parsed;
    try {
    ...
    if (parsed.hostname === "github.com" && segments.length >= 2) {
        ...
        if (segments.length === 2)
            return { ok: true, url: { kind: "github", owner, repo } };
        return { ok: false, error: `unsupported GitHub URL "${input}" — use the repo root or a /tree/<branch>/<dir> link` };
    }
    const pathname = parsed.pathname.toLowerCase();
    if (pathname.endsWith(".zip"))
        return { ok: true, url: { kind: "zip", url: input.trim() } };
    if (pathname.endsWith(".md"))
        return { ok: true, url: { kind: "raw", url: input.trim() } };
...
export async function fetchSkillFiles(input) {
    ...
    if (!classified.ok)
        return { ok: false, errors: [classified.error] };
    ...
    try {
        ...
        if (url.kind === "github" && url.subDir !== undefined) {
            ...
            if (entries.length === 0)
                return { ok: false, errors: [`subdirectory "${url.subDir}" not found in repo archive`] };
        }
        const located = locateSkillRoot(entries.map((e) => e.path));
        if (!located.ok)
            return { ok: false, errors: [located.error] };
...
function githubZipballUrl(owner, repo, ref) {
