...
export function writableRoots(cwd, deps = {}) {
    const roots = [];
    if (cwd !== undefined) {
    ...
    async function list(cwd) {
        ...
        for (const root of roots) {
            ...
            const loaded = await Promise.all(entries.map(async ({ name, format }) => {
                ...
                if (parsed === undefined)
                    return undefined;
                return summaryView(parsed.spec.name, parsed.spec.description, parsed.spec.whenToUse, invocationPolicy(parsed.spec.invocation), root.source, format, false, skillFilePath(root.path, name, format), format === "directory" ? resourceDirectory(root.path, name) : undefined, root.path);
            }));
