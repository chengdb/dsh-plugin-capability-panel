...
/** 声明文件的固定文件名。 */
export const IMPORTS_FILE_NAME = "capability-imports.json" as const;
 ...
 */
export function projectImportsFile(cwd: string, projectRootOverride?: string): string {
  const root = projectRootOverride ?? findProjectRoot(cwd);
  return join(root, ".agents", IMPORTS_FILE_NAME);
}
