 ...
 */
export async function installFromPath(options: InstallFromPathOptions): Promise<TransferResult> {
  const source = await statKind(options.sourcePath);
  if (source === undefined) return { ok: false, errors: [`source "${options.sourcePath}" not found`] };
  ...
  if (parsed === undefined) return { ok: false, errors: ["source is not a valid skill (frontmatter parse failed)"] };
  ...
  if (conflict.errors !== undefined) return { ok: false, errors: conflict.errors, name, existed: true };
 ...
 * 校验通过后才动盘）。
 */
export async function installFromFiles(options: InstallFromFilesOptions): Promise<TransferResult> {
  const files = options.files;
  if (!Array.isArray(files) || files.length === 0) return { ok: false, errors: ["no files uploaded"] };
  if (files.length > MAX_UPLOAD_FILES) return { ok: false, errors: [`too many files (${files.length} > ${MAX_UPLOAD_FILES})`] };
  ...
  for (const file of files) {
    ...
    if (pathError !== undefined) return { ok: false, errors: [pathError] };
    if (seen.has(file.path)) return { ok: false, errors: [`duplicate file "${file.path}"`] };
    seen.add(file.path);
    if (typeof file.content !== "string") return { ok: false, errors: [`file "${file.path}" has no base64 content`] };
  ...
  if (parsed === undefined) return { ok: false, errors: ["uploaded skill file failed frontmatter parsing"] };
  const name = parsed.spec.name;
  if (!isSkillName(name)) {
...
async function collectFiles(baseDir: string, dir: string, out: TransferFile[]): Promise<void> {
 ...
 */
async function resolveConflict(root: string, name: string, overwrite: boolean | undefined): Promise<{ errors?: string[]; existed: boolean }> {
  const existing = await detectFormat(root, name);
  if (existing === undefined) return { existed: false };
  if (overwrite !== true) return { errors: [`skill "${name}" already exists`], existed: true };
...
function validateRelativePath(path: unknown): string | undefined {
  if (typeof path !== "string" || path.length === 0) return "file path must be a non-empty string";
  if (path.includes("\\")) return `invalid path "${path}" — use "/" separators`;
  if (path.startsWith("/") || /^[a-zA-Z]:/.test(path)) return `absolute path "${path}" is not allowed`;
...
async function statKind(path: string): Promise<"file" | "directory" | undefined> {
  try {
