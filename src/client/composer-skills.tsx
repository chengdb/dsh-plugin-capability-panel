 ...
 *   - `conversation.input.left`：ComposerSkillsButton —— MCP 按钮左侧的
 *     闪电图标按钮（order 5 < MCP 的 10）；草稿里不含已知 `/skill` 口令时
 *     空心描边，含已知口令时实心填充绿色（成功色）；
 *   - `conversation.input.overlay`：ComposerSkillsOverlay —— InputBar 浮动
 *     锚点里的弹层，打开时以按钮左上角为锚，容器与行样式对齐宿主 slash
 *     菜单（MenuView）那一族设计变量，按 当前项目/全局 分组列出
 *     user-invocable 的 skill（名称 + 描述单行省略）。
 *
...
/** 记录按钮的视口位置（在打开弹层前调用）；紧随的 setComposerSkillsOpen 会统一派发。 */
export function setComposerSkillsAnchor(rect: { left: number; top: number }): void {
  anchorRect = { left: rect.left, top: rect.top };
}

/** 订阅开合状态（useState + 手动订阅，等价于 mini useSyncExternalStore）。 */
function useComposerSkillsOpen(): { open: boolean; token: number; anchor: { left: number; top: number } | undefined } {
...
export function ComposerSkillsButton({ api, draft }: { api: CapabilityPanelApi; draft: string }) {
  ...
  return (
    <button
      ...
      aria-expanded={open}
      onClick={(event) => {
        setComposerSkillsAnchor(event.currentTarget.getBoundingClientRect());
