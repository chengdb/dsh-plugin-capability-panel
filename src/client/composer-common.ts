...
/** 建一份独立的开合存储。 */
export function createComposerStore<T = unknown>(): ComposerStoreApi<T> {
  ...
  let dataToken = -1;
  let dataWorkspace: string | undefined;
...
export function useComposerStore<T>(store: ComposerStoreApi<T>): ComposerStoreState<T> {
 ...
 */
export function useComposerDismiss<T>(store: ComposerStoreApi<T>, open: boolean, popRef: { current: HTMLElement | null }, ownButtonClass: string): void {
  useEffect(() => {
    if (!open) return;
 ...
 */
export function composerPopStyle(anchor: ComposerAnchor | undefined): CSSProperties | undefined {
 ...
 */
export function useComposerData<T>(state: ComposerStoreState<T>, workspace: string): T | undefined {
