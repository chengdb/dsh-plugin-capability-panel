...
import type { CapabilityPanelApi, ClientMcpList, ClientMcpServer, OpResult } from "./api.js";
import {
  composerPopStyle,
  createComposerStore,
  useComposerData,
...
const store = createComposerStore<ClientMcpList>();
...
function hasEnabledServer(list: ClientMcpList | undefined): boolean {
 ...
 */
async function toggleServer(api: CapabilityPanelApi, server: ClientMcpServer): Promise<OpResult> {
...
function switchTitle(server: ClientMcpServer): string {
    ...
    return (
      <div key={id} className="skp-composer-row">
        <span
          ...
          title={
            server.disabledInProject === true
              ? "本项目禁用（本项目的 session 不挂载）"
              : mount === undefined
                ? "未挂载到当前会话"
                : mount.error ?? MOUNT_LABEL[mount.state]
          }
        />
        <span className="skp-composer-name">{server.key}</span>
        <label className="skp-switch" title={switchTitle(server)}>
          <input type="checkbox" checked={checked} disabled={busy} onChange={() => void onToggle(server)} />
