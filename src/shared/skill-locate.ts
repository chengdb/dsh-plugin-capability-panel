...
/** 定位结果：skill 根目录前缀 + 布局。 */
export interface LocatedSkillRoot {
  /** skill 根目录前缀（posix 风格，不带尾斜杠；包根本身即根时为 ""）。 */
  baseDir: string;
  /** flat = 单 .md；directory = 含 SKILL.md 的目录。 */
  kind: "flat" | "directory";
}

/** 定位失败时给出人读错误文案（候选列表会拼进文案里）。 */
export type LocateResult = { ok: true; root: LocatedSkillRoot } | { ok: false; error: string };

/**
 * 在相对路径列表中定位 skill 根。路径约定为 posix 风格（`/` 分隔）、
 * 不含 `./` 或 `..` 段（调用方在解压/收集时已规范化）。
 */
export function locateSkillRoot(paths: string[]): LocateResult {
  ...
  if (flatCandidates.length === 1) return { ok: true, root: { baseDir: "", kind: "flat" } };
 ...
 */
export function stripCommonTopFolder(paths: string[]): string[] {
 ...
 */
export function rerootEntries<T extends { path: string }>(entries: T[], root: LocatedSkillRoot): T[] {
  if (root.kind === "flat") {
