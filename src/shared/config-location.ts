...
export const PROJECT_CONFIG_DIRS = [".agents", ".dsh", ".claude"] as const;

/** 全局级候选基准目录名（依次拼到 agents-home / dsh-home / 用户主目录下）。 */
export const GLOBAL_CONFIG_DIRS = [".agents", ".dsh", ".claude"] as const;
 ...
 * @param fileName 文件名
 */
export function locateConfigFile(dirs: readonly string[], fileName: string): ConfigFileLocation {
 ...
 */
export function globalConfigBaseDirs(agentsHome?: string, dshHome?: string): string[] {
  const roots: Record<(typeof GLOBAL_CONFIG_DIRS)[number], string> = {
